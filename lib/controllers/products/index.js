...
function _searchProducts(term, next)  {
	...
	db.getConnection(ok(next, function(conn) {
		...
		_.each(keywords, function(word) {
			var likeString = "(products.name LIKE \"%" + word + "%\" OR " +
				"products.barcode LIKE \"%" + word + "%\" OR " +
		...
		var sql = "SELECT products.id " +
			...
			"WHERE " +
			"brands.id = products.brandID AND " +
			"categories.id = products.categoryID AND" +
			likeStatementsString +
			" LIMIT 50";
